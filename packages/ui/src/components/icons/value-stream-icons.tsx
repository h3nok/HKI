...
const BLUE = "#0066B2"; // HKI Blue — primary structure
const RED  = "#E31837"; // HKI Red  — accent / defining detail
...
export function FreshIcon({ size = 20, className }: VSIconProps) {
  return (
    <svg {...svgProps(size, className)}>
      <path d="M12 11C9 8 6 7 4 4c3 0 6 2 8 7Z" stroke={BLUE} fill={`${BLUE}18`} />
      <path d="M12 11c3-3 6-4 8-7-3 0-6 2-8 7Z" stroke={BLUE} fill={`${BLUE}18`} />
      {/* Red center vein */}
...
export function StoreIcon({ size = 20, className }: VSIconProps) {
  return (
    <svg {...svgProps(size, className)}>
      ...
      {/* Red awning */}
      <path d="M3 6h18v3l-1.5 2H4.5L3 9V6z" stroke={RED} fill={`${RED}18`} />
    </svg>
  );
}
...
export function TruckIcon({ size = 20, className }: VSIconProps) {
  return (
    <svg {...svgProps(size, className)}>
      ...
      {/* Red cab + wheels */}
      <path d="M15 10h4l3 3v6h-7V10z" stroke={RED} fill={`${RED}10`} />
...
export function ChartIcon({ size = 20, className }: VSIconProps) {
  return (
    <svg {...svgProps(size, className)}>
      ...
      {/* Red leading bar */}
      <rect x="16" y="6"  width="4" height="16" stroke={RED} fill={`${RED}18`} />
    </svg>
  );
}
