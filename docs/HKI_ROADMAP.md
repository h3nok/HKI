...
| Black-box HTTP-level conformance probe (`hki probe`)          | High               | Track 2 / M4 |
...
| M7  | `hki probe` CLI     | ✅ Scaffold done — `hki-probe` ships in `@hki/conformance` with 10 HTTP probes (P01–P10) and signed JSON evidence bundle output.                                                                                                 |
...
| ID  | Title                            | DoD                                                                                                                                         |
| --- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| M21 | Public site                      | `hki.dev` (or equivalent) live: spec, conformance kit link, threat catalog, registry.                                                       |
| M22 | "Break a RAG in 60 seconds" demo | Repo + 2-minute video taking a vanilla LangChain/LlamaIndex RAG, showing 3 failures from threat catalog, then adding `@hki/runtime` to fix. |
| M23 | Three blog posts                 | (1) "Domain-aware RAG isn’t isolation"; (2) "The semantic cache leak nobody notices"; (3) "HKI as an MCP profile".                          |
