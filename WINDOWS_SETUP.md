...
1. **docker-compose/.env** - Set your GCP project:
   ```
...
Edit `knowledge-api/src/adapters/alloydb_store.py` (around line 109):
...
- retail-agentic-postgres (port 9432)
- retail-agentic-mysql (port 9306)
- retail-agentic-redis (port 9379)
- retail-agentic-neo4j (ports 9474/9687)
- retail-agentic-litellm (port 9400)
- retail-agentic-langfuse (port 3100)
- retail-agentic-langfuse-db
- retail-agentic-pubsub (port 9085)
...
**Solution:** Edit `knowledge-api/src/adapters/alloydb_store.py`:
```python
schema_sql = schema_path.read_text(encoding="utf-8")
```
...
### Code Files Modified:
1. `knowledge-api/src/adapters/alloydb_store.py` - Added UTF-8 encoding to schema.sql read
