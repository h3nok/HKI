...
def _get_store(request: fastapi.Request) -> src.adapters.database.EventStoreProtocol:
    ...
    store: src.adapters.database.EventStoreProtocol | None = getattr(request.app.state, "event_store", None)
    if store is None:
...
async def token_usage(
    ...
    for e in usage_events:
        ...
        if model not in by_model:
            by_model[model] = {"input_tokens": 0, "output_tokens": 0, "embedding_tokens": 0, "calls": 0}
