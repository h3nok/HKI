...
Verifies short-lived HS256 JWTs signed by the Agentic BFF. Every protected
route should use ``Depends(verify_request_jwt)`` to extract the caller's
identity and scope claims.

Canonical source — ALL services import from this module via the
``hki-shared`` package.  Do NOT copy this file into individual services.
...
# Optional post-authentication hook — services can set this to forward the
# bearer token for downstream service-to-service calls.
_on_authenticated: Callable[[str], None] | None = None


def set_on_authenticated_hook(fn: Callable[[str], None]) -> None:
    """Register a callback invoked with the raw bearer token after auth."""
    global _on_authenticated
    _on_authenticated = fn
...
def _get_secret() -> str:
    ...
    try:
        from shared.gcp_secrets import (
            get_secret as sm_get,  # type: ignore[import-untyped]
        )
...
def _normalize_scope_claims(payload: dict[str, typing.Any]) -> tuple[str, list[str]]:
...
def _notify_authenticated(token: str) -> None:
    if _on_authenticated is not None:
        _on_authenticated(token)
...
def _emit_auth_audit(request: fastapi.Request, identity: RequestIdentity) -> None:
    ...
    try:
        ...
        asyncio.ensure_future(  # noqa: RUF006 — intentional fire-and-forget audit
            publish_event(
                "agent-events",
                {
