...
class MCPToolRegistry:
    ...
    async def execute(
        self,
        call: src.domain.models.ToolCallRequest,
    ) -> src.domain.models.ToolCallResult:
        """Execute a tool via its MCP server. Returns result or error."""
        entry: tuple[str, mcp.types.Tool] | None = self._tools.get(call.name)
        if entry is None:
...
def _mcp_tool_to_tooldef(mcp_tool: mcp.types.Tool) -> src.domain.models.ToolDef:
    ...
    parameters: dict[str, typing.Any] = mcp_tool.inputSchema or {
        "type": "object",
        "properties": {},
    }
