...
class AlloyDBVectorStore:
    ...
    def _acl_access_allowed(
        ...
        access_control: src.domain.models.DocumentAccessControl = (
            doc.metadata.access_control or src.domain.models.DocumentAccessControl()
        )
        classification: src.domain.models.DocumentClassification = doc.metadata.classification
        ...
        *,
        ...
        for result in results:
            if result.document_id not in authorized_docs:
                doc: src.domain.models.Document | None = await self._get_document_in_conn(
                    conn,
                    result.document_id,
                    org_id=org_id,
    ...
    async def store_document(
        ...
        async with pool.acquire() as conn, conn.transaction():
            ...
            # Insert chunks (batch)
            chunk_records: list[tuple[str, str, Any | str, str, str | None, int, int, int, int, str]] = [
                (
                    chunk.id,
                    chunk.document_id,
                    org_id,
    ...
    async def update_document_status(
        ...
        existing: src.domain.models.Document | None = await self.get_document(
            document_id,
            org_id=org_id,
            allowed_streams=allowed_streams,
            user_id=user_id,
    ...
    async def update_document_metadata(
        ...
        if not existing:
            return False
        merged: dict[str, Any] = existing.metadata.model_dump(mode="python")
    ...
    async def list_tags(
        ...
        tags: set[str] = set()
        for row in rows:
            doc: src.domain.models.Document = _row_to_document(row)
    ...
    async def search(
        ...
        results: list[src.domain.models.SearchResult] = []
        actual_mode: src.domain.models.SearchMode = query.mode

        org_id: typing.Any | str = getattr(query, "org_id", "default")
        fetch_top_k: int = max(query.top_k * 5, query.top_k)
        ...
        async with pool.acquire() as conn:
            if query.mode == src.domain.models.SearchMode.VECTOR and query_embedding:
                ...
                actual_mode: src.domain.models.SearchMode = src.domain.models.SearchMode.KEYWORD
                results = await self._keyword_search(
                    conn,
                    query.query,
                    query.filters,
            ...
            if query.include_citations:
                for result in results:
                    citation: src.domain.models.Citation | None = await self._build_citation(conn, result)
                    if citation:
    ...
    async def _hybrid_search(
        ...
        keyword_results: list[SearchResult] = await self._keyword_search(
            conn,
            query,
            filters,
            fetch_k,
        ...
        for chunk_id in all_chunk_ids:
            ...
            v_rrf: float = src.core.config.settings.VECTOR_WEIGHT / (k + v_rank)
            k_rrf: float = src.core.config.settings.BM25_WEIGHT / (k + k_rank)
            combined: float = v_rrf + k_rrf

            v_score: float = next((r.vector_score for r in vector_results if r.chunk_id == chunk_id), 0.0)
            k_score: float = next(
        ...
        for chunk_id, score, v_score, k_score in rrf_scores[:top_k]:
            source_result: src.domain.models.SearchResult | None = result_lookup.get(chunk_id)
    ...
    async def _build_citation(
        ...
        preview: str = result.content[:200] + "..." if len(result.content) > 200 else result.content
        ...
        async with pool.acquire() as conn:
            ...
            if not chunk_row:
                return src.domain.models.GraphNeighbors(
                    center_node=src.domain.models.GraphNode(
                        id=chunk_id,
                        label="Unknown",
                    )
                )
            center_doc: src.domain.models.Document | None = await self._get_document_in_conn(
                conn,
            ...
            for row in edge_rows:
                neighbor_id = (
                    row["target_id"] if row["source_id"] == chunk_id else row["source_id"]
                )
                neighbor_doc: src.domain.models.Document | None = await self._get_document_in_conn(
                    conn,
                    row["document_id"],
                    org_id=org_id,
                    allowed_streams=allowed_streams,
...
def _document_custom_metadata(document: src.domain.models.Document) -> dict[str, typing.Any]:
    ...
    custom: dict[str, Any] = dict(document.metadata.custom or {})
...
def _title_search_vector_expr(column_prefix: str = "d.") -> str:
    ...
    prefix: str = column_prefix or ""
    title_expr: str = _normalized_text_expr(f"COALESCE({prefix}title, '')")
...
def _build_filter_clause(
    ...
    # Governance gate: filter by document status. Use provided statuses or
    # default to published/indexed.
    ...
    if filters.document_types:
        types: list[str] = [dt.value if hasattr(dt, "value") else str(dt) for dt in filters.document_types]
...
def _row_to_document(row: asyncpg.Record) -> src.domain.models.Document:
    ...
    try:
        access_control: src.domain.models.DocumentAccessControl = src.domain.models.DocumentAccessControl.model_validate(
