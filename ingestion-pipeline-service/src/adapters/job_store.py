...
class JobStore(typing.Protocol):
    ...
    async def list(self, limit: int = 50, offset: int = 0) -> tuple[list[src.domain.models.IngestionJob], int]: ...
...
class RedisJobStore:
    ...
    def __init__(self, redis_url: str) -> None:
        ...
        self._prefix = src.core.config.settings.REDIS_JOB_PREFIX
        self._index_key: str = f"{self._prefix}index"
        self._ttl = src.core.config.settings.REDIS_JOB_TTL_HOURS * 3600
    ...
    async def recover_stale_jobs(self) -> int:
        ...
        """
        terminal = {src.domain.models.JobStatus.COMPLETED.value, src.domain.models.JobStatus.FAILED.value}
    ...
    async def hvsi_audit(self) -> dict[str, int]:
        return {
            "jobs_missing_stream": sum(
                1 for job in self._jobs.values() if not str(job.stream_id or "").strip()
            )
        }
