...
async function buildLaunchReadinessState(
  ...
  const [
    ...
    serviceJsonTyped<ReviewRecord[]>(
      `${KNOWLEDGE_PIPELINE_URL}/v1/review/pending?limit=100`,
      ctx
...
export const knowledgeRouter = router({
  ...
  taxonomyCreateNode: managerProcedure
    ...
    .mutation(async ({ input, ctx }): Promise<TaxonomyNode> => {
      ...
      return serviceJsonTyped<TaxonomyNode>(
        ...
        {
          method: "POST",
          body: buildSnakeBody(input),
        }
      );
    }),

  /** List root taxonomy nodes. */
  taxonomyListRoots: managerProcedure.query(
    async ({ ctx }): Promise<TaxonomyNode[]> => {
      await requireKnowledgeFeatureFlag(
        ctx,
        "release.knowledge.library.taxonomy",
        "Taxonomy is disabled for this deployment"
  ...
  taxonomyDeleteNode: managerProcedure
    .input(z.object({ nodeId: z.string(), cascade: z.boolean().optional() }))
  ...
  taxonomyMoveNode: managerProcedure
    .input(z.object({ nodeId: z.string(), newParentId: z.string().nullable() }))
    .mutation(async ({ input, ctx }): Promise<TaxonomyNode> => {
      await requireKnowledgeFeatureFlag(
        ctx,
        "release.knowledge.library.taxonomy",
  ...
  taxonomyValidateTags: managerProcedure
    .input(z.object({ tags: z.array(z.string().max(64)).max(50) }))
  ...
  taxonomyInfo: managerProcedure.query(
    async ({ ctx }): Promise<TaxonomyTreeInfo> => {
      await requireKnowledgeFeatureFlag(
        ctx,
        "release.knowledge.library.taxonomy",
        "Taxonomy is disabled for this deployment"
  ...
  observabilitySummary: knowledgeReadProcedure.query(
    async ({ ctx }): Promise<TraceSummary> => {
      try {
