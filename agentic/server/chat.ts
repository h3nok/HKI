...
function getDownstreamScopes(activeScope: string): string[] {
  return activeScope === "global" ? ["global"] : [activeScope];
}
...
function getAssistantFailureMessage(error: unknown): string {
  const timeoutMessage =
    "I hit an upstream timeout before I could finish. Please try again.";

  if (!(error instanceof Error)) return timeoutMessage;
  ...
  return "I ran into a problem while generating the response. Please try again.";
}
...
async function processConversationMessageStream({
  ...
  try {
    ...
    const authToken = await signRequestJwt(user, activeScope, downstreamScopes);
  ...
  sendMessage: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!db) {
        ...
        finalContent = devFinalContent || devLiveResponseContent;
