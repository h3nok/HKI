...
const SIDEBAR_STORAGE_KEY = "agentic.sidebar_state";
...
function getStoredSidebarState(defaultOpen: boolean) {
  if (typeof window !== "undefined") {
    try {
      const storedState = parseSidebarState(
      ...
      // Ignore storage access failures and fall back to cookie/default state.
    }
  }

  const cookieState = parseSidebarState(getCookieValue(SIDEBAR_COOKIE_NAME));
  return cookieState ?? defaultOpen;
}

function persistSidebarState(open: boolean) {
  const serializedState = String(open);

  if (typeof window !== "undefined") {
    try {
      window.localStorage.setItem(SIDEBAR_STORAGE_KEY, serializedState);
    } catch {
      // Ignore storage access failures and keep the in-memory state working.
    }
  }

  if (typeof document !== "undefined") {
    document.cookie = `${SIDEBAR_COOKIE_NAME}=${encodeURIComponent(serializedState)}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}; SameSite=Lax`;
  }
}
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    ...
    const handleStorage = (event: StorageEvent) => {
      ...
      const nextOpen = parseSidebarState(event.newValue);
