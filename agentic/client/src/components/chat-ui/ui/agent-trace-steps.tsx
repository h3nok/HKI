...
const STEP_CONFIG: Record<string, { icon: typeof Brain; label: string; color: string }> = {
  ...
  memory_recall:       { icon: Database,     label: 'Memory recall',          color: 'text-violet-500' },
  thinking:            { icon: Brain,        label: 'Reasoning',              color: 'text-amber-500' },
  planning:            { icon: Sparkles,     label: 'Planning',               color: 'text-cyan-500' },
  tool_call:           { icon: Wrench,       label: 'Tool call',              color: 'text-orange-500' },
  tool_result:         { icon: CheckCircle2, label: 'Tool result',            color: 'text-emerald-500' },
  knowledge_retrieval: { icon: BookOpen,     label: 'Knowledge search',       color: 'text-indigo-500' },
  reflecting:          { icon: Sparkles,     label: 'Synthesizing',           color: 'text-purple-500' },
  memory_store:        { icon: Database,     label: 'Memory update',          color: 'text-violet-400' },
  cache_hit:           { icon: Clock,        label: 'Cache hit',              color: 'text-green-500' },
};

const DEFAULT_CONFIG = { icon: Brain, label: 'Working', color: 'text-muted-foreground' };
...
function formatStepContent(step: TraceStep): string {
  ...
  if (type === 'guardrail' && metadata?.report) {
    return metadata.report.passed ? 'Input validated — safe to proceed' : 'Input flagged by safety check';
  }
  ...
  if (type === 'prompt_stack' && metadata?.summary) {
    const source = String(metadata.summary.domain_prompt_source || 'runtime').replace(/_/g, ' ');
...
export function AgentTraceSteps({ steps, isStreaming, className = '' }: AgentTraceStepsProps) {
  // Filter out internal/noise steps for a cleaner inline view
  const visibleSteps = useMemo(() =>
    steps.filter(s =>
      s.type !== 'final_response' &&
      s.type !== 'memory_store'
    ),
    [steps],
  );
          ...
          >
            <div className="flex flex-col pl-1 border-l border-border/40 ml-1 space-y-0.5">
              {visibleSteps.map((step, i) => (
                <TraceStepRow key={`${step.type}-${i}`} step={step} index={i} isLast={i === visibleSteps.length - 1} isStreaming={isStreaming} />
              ))}
            </div>
          </motion.div>
        )}
...
function TraceStepRow({ step, index, isLast, isStreaming }: { step: TraceStep; index: number; isLast: boolean; isStreaming: boolean }) {
  ...
  return (
    ...
    >
      <Icon className={`w-3 h-3 shrink-0 ${isActive ? config.color : 'text-muted-foreground/50'}`} />
