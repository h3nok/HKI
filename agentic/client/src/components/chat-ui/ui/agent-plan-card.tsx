...
export function AgentPlanCard({
  ...
  const highestRisk = steps.reduce((max, s) => {
  ...
  return (
    <motion.div
      ...
      style={{
        background: 'var(--card)',
        borderColor: `color-mix(in srgb, var(--warning, #d97706) 30%, var(--border))`,
        boxShadow:
          '0 0 0 1px color-mix(in srgb, var(--warning, #d97706) 10%, transparent), 0 4px 16px -4px color-mix(in srgb, var(--neutral-900) 8%, transparent)',
      }}
    >
      {/* Header */}
      <button
      ...
      >
        ...
        >
          <Zap className="w-4 h-4" style={{ color: 'var(--warning, #d97706)' }} />
        </div>
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className="px-3.5 pb-2 space-y-1.5" style={{ borderTop: '1px solid var(--border)' }}>
                        ...
                        >
                          <div className="px-2.5 pb-2.5 space-y-2">
                            {step.expectedImpact && (
                              ...
                              >
                                <ArrowRight className="w-3 h-3 mt-0.5 shrink-0" style={{ color: 'var(--primary)' }} />
                                <span style={{ color: 'var(--muted-foreground)' }}>
                                  <strong style={{ color: 'var(--foreground)' }}>Expected: </strong>
                                  {step.expectedImpact}
                                </span>
                              </div>
                            )}
                            {step.arguments && Object.keys(step.arguments).length > 0 && (
                              <pre
                                className="text-[11px] p-2 rounded-md overflow-x-auto bg-muted text-muted-foreground"
                              >
