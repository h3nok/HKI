 ...
 *
 * Bold, clear, elevated. Features the HKI ThinkingAnimation (red C +
 * blue orbital) with an enlarged, helpful trace timeline. Professional
 * but unmistakably HKI.
 */
...
function formatStep(step: TraceStep): string {
  ...
  if (type === "knowledge_retrieval")
    return content || "Searching knowledge base";
...
function ToolDetail({
  ...
  return (
    <div className="rounded-lg bg-muted/20 dark:bg-muted/10 border border-border/30 overflow-hidden">
      ...
      >
        <Wrench className="w-3.5 h-3.5 shrink-0" style={{ color: BLUE }} />
...
export function ThinkingEnvelope({
  ...
  const knowledgeLiveSummary = useMemo(() => {
    ...
    if (latestVisibleStep?.type === "knowledge_retrieval") {
      return "Searching the knowledge base for supporting evidence.";
    }
  ...
  const completedSubline = useMemo(() => {
    ...
    return parts.join(", ");
  }, [knowledgeSignals.kbUsed, toolPairs.length, dedupedSections.length]);
      ...
      >
        ...
        >
          <button
            ...
            className="flex items-center gap-3 w-full px-3.5 py-2.5 text-left transition-colors hover:bg-muted/10"
          >
          ...
          <div className="flex-1 min-w-0">
            <AnimatePresence mode="wait">
              <motion.span
                ...
                exit={{ opacity: 0, y: -4 }}
                transition={{ duration: 0.15 }}
                className="block text-[13px] font-semibold tracking-[-0.01em] text-foreground"
              >
                {liveHeadline}
              </motion.span>
            </AnimatePresence>
                ...
                return (
                  ...
                  >
                    {isLast ? (
                      ...
                      <div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0 bg-muted/30">
                        <Check
