...
import { ThinkingAnimation } from "./ui/thinking-animation";
...
function ConversationStartingCard({ scopeName }: { scopeName?: string }) {
  const activeScopeLabel = scopeName || "Employee Copilot";
          ...
          <div className="flex items-start gap-4">
            <div className="relative shrink-0">
              <div
                ...
                style={{
                  background:
                    "radial-gradient(circle at center, color-mix(in srgb, var(--foreground) 5%, transparent) 0%, transparent 72%)",
                }}
              />
              <ThinkingAnimation
                variant="standard"
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                Locking into {activeScopeLabel} and preparing a grounded,
                professional response with live reasoning updates.
              </p>
...
/** Generate scope-aware prompt suggestions from value stream metadata.
...
export function ChatUIRoot({ userId, className }: ChatUIRootProps) {
  ...
  useEffect(() => {
    ...
    for (const scopeDef of availableScopes) {
      ...
      if (!scopeId || !scopeName || scopeId === GLOBAL_SCOPE) continue;
      if (scopeDef.sampleQuestions?.length) continue;
      if (Object.prototype.hasOwnProperty.call(autoPromptsByScope, scopeId))
        continue;
      if (generatingScopePromptsRef.current.has(scopeId)) continue;
      ...
      suggestQuestionsMut.mutate(
        {
          title: `${scopeName} knowledge base`,
          content: [
            `Value stream: ${scopeName}`,
  ...
  return (
    <div
      className={`flex h-screen w-full overflow-hidden ${className || ""}`}
      role="application"
    ...
    >
      ...
      <TaskSidebar
        ...
        activeStreamName={activeScopeDef?.name}
        activeStreamIcon={activeScopeDef?.icon}
      ...
      <main
        ...
        aria-label="Chat"
      >
        {/* Header - Frameless transparent bar */}
        <header
          role="banner"
          aria-label="Chat actions"
          className="shrink-0 flex items-center justify-between px-4 sm:px-6 z-10"
        ...
        >
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            >
              ...
              <ToolbarAction
                asChild
                tooltip="Open Knowledge Base"
                className="platform-nav-link"
              >
                <a href={knowledgeHref} aria-label="Open Knowledge Base">
          ...
          {taskID ? (
            ...
            >
              ...
              <div
                ...
                className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-y-contain flex flex-col items-center"
                role="region"
            ...
            >
              <div className="w-full" style={{ maxWidth: chatMaxWidth }}>
                ...
                >
                  ...
                  >
                    {activeScopeDef && activeScopeDef.id !== GLOBAL_SCOPE
                      ...
                      : "HKI Agent · Employee Copilot"}
                  </span>
                </motion.div>

                {isAgentBusy ? (
                  <ConversationStartingCard scopeName={activeScopeDef?.name} />
                ) : (
                  <>
                    {/* Input */}
                    <motion.div
