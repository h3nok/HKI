...
export type ToolCategory = 'search' | 'data' | 'compute' | 'communication' | 'other';
...
function ToolCard({ tool, onTest }: { tool: Tool; onTest?: (tool: Tool) => void }) {
  ...
  const handleCopyExample = async () => {
    if (tool.example) {
      try {
        await navigator.clipboard.writeText(JSON.stringify(tool.example.input, null, 2));
      } catch {
    ...
    >
      ...
      >
        <div className="flex items-start gap-3">
          ...
          >
            <CategoryIcon className="h-4 w-4" style={{ color: categoryConfig_.color }} />
          </div>
...
export function ToolRegistryBrowser({ tools, onTestTool }: ToolRegistryBrowserProps) {
