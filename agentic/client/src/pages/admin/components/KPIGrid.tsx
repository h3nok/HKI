...
export function KPICardBox({ kpi }: { kpi: KPI; i?: number }) {
  ...
  return (
    <HkiCard
      ...
      className={cn(
        a.metricCard,
        "h-full px-4 py-4 flex flex-col justify-between gap-3 overflow-hidden"
      )}
    >
      <div className="flex items-center justify-between">
        ...
        {kpi.trend != null && kpi.trend !== 0 && (
          <div
            className={cn(
              "flex items-center gap-0.5 text-[10px] font-medium tabular-nums px-1.5 py-0.5 rounded-full border",
        ...
        {kpi.trend === 0 && (
          <div
            className={cn(
              a.pillNeutral,
              "flex items-center gap-0.5 text-[10px] font-bold tabular-nums px-1.5 py-0.5 rounded-full border"
            )}
          >
