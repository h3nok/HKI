...
import { Button, cn, COSTCO_BLUE, COSTCO_RED, StreamIcon } from "@hki/ui";
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-start gap-4">
              <div className="relative shrink-0">
                ...
                <div className="absolute -right-1 -top-1 h-3 w-3 rounded-full bg-secondary ring-2 ring-background" />
              </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// HVSI EXPLAINER — educational callout for stream isolation
// ═══════════════════════════════════════════════════════════════════════════════
...
const HVSI_POINTS = [
  {
    title: "One workspace, one stream",
    detail:
      "Every document, chunk, and retrieval request belongs to exactly one value stream. No cross-stream bleed.",
  },
  {
    title: "Fail closed",
    detail:
      "If a stream is missing or unauthorized the request fails — no silent fallback to a global scope.",
  },
  {
    title: "Shared knowledge uses replication",
    detail:
      "If a policy applies to multiple streams it is published as stream-specific copies, never a shared wildcard record.",
  },
];
    ...
    <div className={cn("mt-6 rounded-2xl", k.card)}>
      ...
      >
        <div
          ...
          style={{
            background: `color-mix(in srgb, ${COSTCO_BLUE} 12%, transparent)`,
          }}
        >
          <ShieldCheck className="h-4 w-4" style={{ color: COSTCO_BLUE }} />
        </div>
        <div className="min-w-0 flex-1">
          <p className={k.heading}>
            <span style={{ color: COSTCO_BLUE }}>Hermetic</span>{" "}
            <span style={{ color: COSTCO_RED }}>Value Stream</span> Isolation
            <span className={cn("ml-1.5", k.muted)}>(HVSI)</span>
          </p>
        </div>
        {isOpen ? (
      ...
      {isOpen ? (
        <div className="space-y-4 border-t border-border/40 px-5 pb-5 pt-4">
          <p className={k.subtext}>
            This platform enforces hermetic isolation — each workspace is a
            sealed boundary. Documents, retrieval, and ingestion never cross
            stream lines at runtime.
          </p>
...
// ═══════════════════════════════════════════════════════════════════════════════
// STREAM LANDING — hermetic entry point for workspace selection
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div className={cn("min-h-svh", k.ground)}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={k.heading}>Knowledge Base</span>
        </div>
        <div className="flex items-center gap-2">
          {canManage ? (
            <Button
      ...
      <div className="mx-auto max-w-5xl px-6 py-10">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {hasStreams
                ? `${streams.length} workspace${streams.length === 1 ? "" : "s"}`
                : "Get started"}
            </div>
          </div>
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">
            Select a <span className={k.accentText}>workspace</span>
          </h1>
          <p className={cn(k.subtext, "max-w-xl")}>
            Knowledge opens one value stream at a time. Pick a workspace below
            to start curating content.
          </p>
        </div>
        ...
        {hasStreams ? (
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {/* Create new stream card (admin only) */}
            {canManage ? (
              <button
                type="button"
                onClick={() => onNavigate("/knowledge/create")}
          ...
          >
            ...
            <div className="max-w-sm text-center">
              <p className={k.heading}>No workspaces available</p>
              <p className={cn(k.muted, "mt-1.5")}>
                {canManage
                  ? "Create a workspace to start curating knowledge for your team."
                  : "Ask an admin to create or assign a workspace for you."}
              </p>
            </div>
            {canManage ? (
              <Button
