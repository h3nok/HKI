/**
 * Knowledge Base — Reusable UI Primitives
 *
...
export type KBHKIEmojiKey = keyof typeof KB_COSTCO_EMOJI;
...
export function KBEmojiBadge({
  ...
  const entry = KB_COSTCO_EMOJI[emoji];
  return (
    <span
      aria-hidden="true"
...
const STATUS_PILL = "border-border/55 bg-background";
...
const SECONDARY_WORKFLOW_ACTION = cn(
  "border border-border/55 bg-background text-foreground shadow-none hover:-translate-y-px hover:border-border/75",
  k.duoToneHover,
  k.duoToneTextHover
);
...
export function KBWorkflowBanner({
  ...
  return (
    <div
      ...
      className={cn(
        cardCls,
        "relative overflow-hidden rounded-t-3xl rounded-b-none border border-border/50 shadow-sm",
        className
      )}
    >
          ...
          <div className="flex min-w-0 flex-1 items-start gap-4">
            ...
            <div className="min-w-0 space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                {eyebrow && (
                  <span className="rounded-full border border-border/55 bg-background px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground/72">
                    {eyebrow}
                  </span>
                )}
                {eyebrow && statusLabel && (
