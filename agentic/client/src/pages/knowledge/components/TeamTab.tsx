...
export default function TeamTab({ selectedStream, streamLabel }: TeamTabProps) {
  ...
  const handleCreate = () => {
    ...
    if (!selectedStream)
      return notify({
        title: "Select a value stream first",
        severity: "warning",
      });
    createMut.mutate({
              ...
              <div className="p-5 space-y-4">
                ...
                {!selectedStream && (
                  <div className="p-3 rounded-xl bg-primary/5 border border-primary/15 text-xs text-primary">
                    Select a value stream from the header dropdown before
                    sending invites.
                  </div>
                )}
            ...
            >
              ...
              {!selectedStream ? (
                <div className="p-5 text-xs text-primary">
                  Select a value stream to review requests.
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <span className="px-1.5 py-0.5 text-xs font-semibold rounded-full uppercase tracking-wider bg-amber-500/10 text-amber-500">
                            pending
                          </span>
                        </div>
        ...
        <div className="space-y-4">
          {canManage && (
            ...
            >
              <div className="border-b border-border/40 px-5 py-3 flex items-center justify-between">
                <h4 className="text-sm font-bold text-foreground flex items-center gap-2">
                  <span id="members-heading">👥 Stream Members</span>
                </h4>
              ...
              <div className="px-5 py-3 text-[11px] leading-4 text-muted-foreground border-b border-border/30 bg-muted/25">
                ...
                <span className="font-semibold text-foreground">
                  {streamLabel || "the selected stream"}
                </span>
                . Role changes apply to the user&apos;s current enterprise tier,
                while stream assignment controls where that tier applies.
              </div>

              {!selectedStream ? (
                <div className="p-5 text-xs text-primary">
                  Select a value stream to manage members.
                </div>
              ) : membersQ.isLoading ? (
                <div className="flex items-center justify-center py-12">
                  <Loader2 className="w-5 h-5 animate-spin text-muted-foreground/60" />
                ...
                <div className="text-center py-10 space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    No members are assigned to this stream yet
                  </p>
                </div>
              ) : (
                <div className="divide-y divide-border/30" role="list">
