...
const CLASSIFICATION_OPTIONS: Array<{
...
}> = [
  {
    value: "internal",
    label: "Internal",
    description: "Normal stream content for the broader working team.",
  },
  {
    value: "confidential",
    label: "Confidential",
        ...
        <div className="rounded-xl border border-border/25 bg-muted/10 p-1.5">
          <div className="grid grid-cols-2 gap-1">
            ...
            >
              <span>Access</span>
              {needsAllowList ? (
                <span className="rounded-full bg-amber-500/12 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-700 dark:text-amber-400">
                  Review
                </span>
          ...
          {needsAllowList && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2.5">
              ...
              <p
                className={cn(
                  k.muted,
                  "mt-1 text-xs leading-5 text-amber-700/80 dark:text-amber-400/80"
                )}
              >
    ...
    <div className="space-y-4">
      ...
      <div className={cn(k.inset, "flex items-start gap-3 p-4")}>
        ...
        <p className={cn(k.muted, "leading-5")}>
          ...
          before it reaches the knowledge base.
        </p>
      </div>
    </div>
  );
            ...
            <div className="flex flex-wrap items-center gap-2 mt-3">
              {isBatch && batchSummary ? (
                <>
                  ...
                  <StatPill label="Rejected" value={`${batchSummary.rejected}`} />
                </>
              ) : (
                <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
          ...
          <div className={cn(k.card, "p-5 lg:col-span-2 space-y-4")}>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="flex flex-col items-center gap-1 rounded-xl bg-amber-500/5 border border-amber-500/15 p-2.5">
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          f.analysisStatus === "flagged" && "bg-amber-500",
                      ...
                      <div className="flex flex-wrap gap-1">
                        {selectedFile.analysis.validate.issues
                          ...
                          .map((issue, i) => (
                            <span
                              key={i}
                              className="inline-flex items-center gap-1 rounded-md bg-amber-500/10 px-2 py-0.5 text-[10px] text-amber-600 dark:text-amber-400"
                            >
        ...
        <>
              ...
              <div className="flex items-start gap-3">
                <AlertTriangle
                  className={cn(
                    ...
                    hasCriticalPII ? "text-red-500" : "text-amber-500"
                  )}
                />
                <div className="min-w-0">
                  <h4 className="text-sm font-semibold text-foreground">
                  ...
                  <div className="mt-2 space-y-2">
                    ...
                    {hasDupes && (
                      <p className="text-sm text-foreground/85">
                        {hasExactDupe
                          ? "This file is an exact duplicate of content already in the knowledge base"
                          : "This looks similar to content already in the knowledge base"}
                        {duplicatePreview
                    ...
                    {missingAclForSensitiveDoc && (
                      <p className="text-sm text-foreground/85">
                        ...
                        normal stream access. Add an allow-list before indexing
            ...
            <div className={cn(k.card, "p-4")}>
              ...
              <div className="space-y-2">
                ...
                <DetailRow
                  ...
                  valueColor={hasDupes ? "text-amber-500" : "text-emerald-500"}
                />
          ...
          {decide.recommendation === "review" && mode === "text" && (
            <div
              className={cn(
                k.card,
                "overflow-hidden border border-amber-500/15"
              )}
            >
                  ...
                  {validate.issues.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {validate.issues.slice(0, 4).map((issue, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center gap-1 rounded-lg bg-amber-500/10 px-2.5 py-1 text-[11px] text-amber-600 dark:text-amber-400"
                        >
        ...
        <div className="flex items-center gap-2">
          {isBatch && batchSummary ? (
            ...
            >
              ...
              {uploadableFiles.length > 0
                ? `Add ${uploadableFiles.length} of ${batchSummary.total} to Knowledge Base`
            ...
            <Button
              ...
              size="lg"
              className="text-sm font-bold gap-2 border-amber-500/30 text-amber-700 dark:text-amber-400 hover:bg-amber-500/10"
            >
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
              <motion.div
                className={cn(
                  "h-full rounded-full",
                  errCount > 0 ? "bg-amber-500" : "bg-primary"
                )}
                initial={{ width: 0 }}
                animate={{
...
function CompletePhase({
  ...
  const StatusIcon = completionState === "ready"
    ? CheckCircle
    : completionState === "pending_review"
      ...
      : isFailed
        ? AlertTriangle
        : Loader2;
  const statusIconClass = completionState === "ready"
    ? "text-emerald-500"
    : completionState === "pending_review"
      ? "text-amber-500"
      : completionState === "archived"
  ...
  const statusShellClass = completionState === "ready"
    ? "bg-emerald-500/15"
    : completionState === "pending_review"
      ? "bg-amber-500/12"
      : completionState === "archived"
      ...
      : isFailed
        ? "bg-red-500/12"
        : "bg-primary/10";
  const statusBadgeLabel = completionState === "ready"
    ...
    : completionState === "pending_review"
      ...
      : isFailed
        ? "Not completed"
        : "In progress";
  const heading = completionState === "ready"
  ...
  const description = completionState === "ready"
    ? `“${resolvedTitle}” is ${completedDocumentStatus === "published" ? "published" : "indexed"} in this stream and ready to ground answers.`
    : completionState === "pending_review"
      ? `“${resolvedTitle}” was added to this stream, but it will not ground answers until a reviewer approves and publishes it.`
      : completionState === "archived"
  ...
  const nextSteps = completionState === "ready"
    ? []
    : completionState === "pending_review"
      ? [
              ...
              <div className="rounded-xl border border-border/25 bg-background/70 px-3 py-3">
                ...
                <p className={cn(k.muted, "mt-1 leading-5")}>
                  {hasExplicitAccessControl(completedAccessControl)
                    ...
                    : "No extra document ACL was added beyond the current stream boundary."}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          {isFailed && hasDocument && onRetry && (
            <button
              ...
              disabled={isRetrying}
              className="p-1.5 rounded-lg hover:bg-amber-500/10 text-amber-500/60 hover:text-amber-500 transition-colors disabled:opacity-40"
              ...
              <div className="flex items-center gap-2 mt-3 pt-2 border-t border-border/30">
                ...
                {isFailed && hasDocument && onRetry && (
                  <button
                    onClick={() => onRetry(job.documentId)}
                    disabled={isRetrying}
                    className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-amber-600 dark:text-amber-400 hover:bg-amber-500/10 rounded-lg transition-colors disabled:opacity-40"
                  >
                    {isRetrying ? (
                      <Loader2 className="w-3 h-3 animate-spin" />
                    ) : (
