...
export default function LLMOperationsCard() {
  ...
  const errorTone = errorRate > 5 ? "critical" : errorRate > 0 ? "warning" : "positive";
  ...
  const totalOutputTokens = (obs as any).totalOutputTokens as number | undefined;
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            cls: obs.errorCount > 0 ? k.statusCriticalText : "text-muted-foreground",
          },
          {
            label: "Avg latency",
            value: obs.avgDurationMs ? `${obs.avgDurationMs.toFixed(0)}ms` : "—",
            icon: Clock,
          ...
          },
          {
            ...
            icon: Clock,
            cls: (p95DurationMs ?? 0) > 5000 ? k.statusWarningText : "text-muted-foreground",
          },
        ].map(s => (
          <div key={s.label} className="space-y-1">
            <p className={cn("text-xl font-black tabular-nums leading-none", s.cls)}>
              {s.value}
            </p>
