/**
 * Enhanced BrowseView — Knowledge Base Document Repository
 *
...
function getDocumentReviewBlocker(doc: any, selectedStream?: string) {
  ...
  if (!resolveDocumentReviewStreamId(doc, selectedStream)) {
    return "Assign a value stream before sending this document for review.";
  }
...
function getDocumentStatusDisplay(docStatus: string | null | undefined) {
  ...
  if (normalizedStatus === "pending_review") {
    return { label: "Pending Review", color: "text-orange-600" };
  }
    ...
    void (async () => {
      ...
      if (successes === 0 && skipped > 0 && failures === 0) {
        notify({
          ...
          description:
            "Select published documents with a title and stream assignment, then try again.",
          severity: "warning",
          group: "knowledge",
        });
        return;
        ...
        <div className="text-center py-24 space-y-3 bg-card/60 dark:bg-card/40 rounded-2xl border border-dashed border-border/50 dark:border-border">
          ...
          <p className={cn("text-sm max-w-sm mx-auto", k.muted)}>
            {filterText
              ...
              : selectedStream
                ? `No documents are available in ${streamLabel || selectedStream} yet. Upload or publish content for this stream to populate the library.`
