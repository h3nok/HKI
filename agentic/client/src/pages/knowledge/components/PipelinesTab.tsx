      ...
      >
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className={cn(k.subtext, "max-w-sm mb-6")}>
            New content appears here while it is being prepared for the
            knowledge base. Start by adding some content first.
          </p>
          {onNavigate && (
              ...
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border/30">
                ...
                {isFailed && hasDocument && onRetry && (
                  <button
                    ...
                    disabled={isRetrying}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg text-amber-600 dark:text-amber-400 bg-amber-500/10 hover:bg-amber-500/18 transition-colors disabled:opacity-40"
                  >
                    {isRetrying ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
