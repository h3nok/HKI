 ...
 * Usage:
 *   import { k } from '../theme';
 *   <h2 className={k.display}>Knowledge Base</h2>
 */

import { COSTCO_BLUE, COSTCO_RED, brandColors } from "@hki/ui";

// ── Primary accent — sourced from @hki/ui ─────────────────────────────────

/** Hex values for inline styles (icon boxes, SVG strokes, etc.) */
export const ACCENT = COSTCO_BLUE;
export const ACCENT_RED = COSTCO_RED;
...
export const k = {
  ...
  accentBar: [
    ...
    "before:rounded-full before:bg-gradient-to-b before:from-primary before:to-secondary",
  ].join(" "),

  // ── Glow — subtle brand halo for focused/active elements ──
  glow: "shadow-[0_0_20px_rgba(0,102,178,0.15)] dark:shadow-[0_0_20px_rgba(51,153,204,0.2)]",
} as const;
 ...
 * Each value stream gets a deterministic slot so the color is stable
 * across sessions — same stream always gets the same color.
 *
