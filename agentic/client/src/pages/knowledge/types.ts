...
> = {
  ...
  enriching: { Icon: Loader2, color: "text-amber-500" },
...
export function getFreshness(referenceAt: string | null): {
  ...
  if (days < 90)
    return { label: "Review", color: "text-amber-500 bg-amber-500/10" };
...
> = {
  internal: {
    label: "Internal",
    description: "Standard content for the current value stream.",
    tone: "neutral",
  },
  confidential: {
    label: "Confidential",
...
export function summarizeAccessControl(
  ...
  if (!allowed.length && !denied.length) {
    return options?.emptyLabel ?? "Uses normal stream access";
  }
