...
HKI ships a working standard, two language runtimes, a 28-case conformance kit, a 15-entry threat catalog with runnable demos, six framework adapters (LiteLLM, LangChain, LlamaIndex, AutoGen, CrewAI, Google ADK), a CI/CD conformance action, an evidence registry, and a full production reference platform — validated across **14 enterprise domains at a Fortune 15 retailer**.

Conformance level of the reference platform: **L3** (28/28 adapter cases pass, HTTP probe runner ships in next release).
...
| 2 | **Python conformance kit (M2.2)** | Python `hki-conformance` CLI mirrors all 28 cases; `hki-conformance-py` runs via `pytest` | Q2 2026 |
| 3 | **L4 probe (M7 full)** | `hki probe` HTTP suite passes against deployed reference platform; signed evidence bundle | Q2 2026 |
