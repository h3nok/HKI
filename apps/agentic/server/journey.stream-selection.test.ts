...
function makeUser(overrides: Partial<User> = {}): User {
  return {
    id: 1,
    openId: "journey-stream-user",
    name: "Knowledge Manager",
    email: "journey.manager@hki.com",
    loginMethod: "google",
    pngId: null,
    role: "manager",
    department: null,
...
describe("journey stream selection", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getDb.mockResolvedValue({});
  });

  it("fails closed when journey progress omits a stream", async () => {
    const { appRouter } = await import("./routers");
    const caller = appRouter.createCaller(makeCtx());

    await expect(
      caller.journey.getProgress({ valueStreamId: "" })
    ).rejects.toThrow(
      /Select a value stream before using the Knowledge journey/
    );
  });

  it("rejects global journey access even for admins", async () => {
    const { appRouter } = await import("./routers");
    ...
    await expect(
      caller.journey.getProgress({ valueStreamId: "global" })
