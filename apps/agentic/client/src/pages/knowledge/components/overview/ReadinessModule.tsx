import { cn, HKI_IRIS, Popover, PopoverTrigger, PopoverContent } from "@hki/ui";
...
const LEVEL_EXPLANATION: Record<MaturityLevel, string> = {
  seed: "This domain is just getting started. Add content and complete the setup steps to improve readiness.",
  sprout:
  ...
  elite:
    "Top-tier agentic readiness. This domain is production-grade with comprehensive coverage and quality.",
};
...
const DIMENSION_TIPS: Record<string, string> = {
  ...
  Adoption:
    "Grow usage — more queries and users interacting with the domain library.",
};
...
export function ReadinessModule({
  ...
  const summaryLabel = hasEvidence
    ? `${MATURITY_LEVELS[maturity.level].label} domain`
    : "New domain";
  ...
  const statusTextClass = launchStatus
    ? launchStatus.tone === "success"
      ...
      : launchStatus.tone === "warning"
        ? "text-primary"
        : launchStatus.tone === "danger"
          ? "text-destructive"
    ...
    <Popover>
      ...
      >
        ...
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${HKI_IRIS}, ${HKI_IRIS}dd)`,
          }}
        >
          <p className="text-sm font-bold">
            {MATURITY_LEVELS[maturity.level].label} Domain — {maturity.overall}
            /100
          </p>
        ...
        <div className="px-4 py-3 space-y-3">
          ...
          {hasEvidence && (
            <div className="grid grid-cols-2 gap-2">
              ...
              {weakest && (
                <div className="rounded-lg bg-primary/8 px-2.5 py-2">
