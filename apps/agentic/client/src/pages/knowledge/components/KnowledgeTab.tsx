        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className={cn("text-xl font-bold", k.heading)}>
              {view === "browse" ? "Knowledge Domain" : "Review Queue"}
            </h1>
            <p className={cn("text-sm mt-0.5", k.muted)}>
              {view === "browse"
                ? "Browse and manage all indexed documents."
                : "Approve, revise, or reject submitted documents."}
            </p>
          </div>
          {/* ── Mode switcher ── */}
          <div className="flex items-center gap-1 p-1 rounded-xl bg-black/3 dark:bg-muted/40">
            ...
            >
              ...
              {pendingCount > 0 && (
                <span className="px-1.5 py-0.5 rounded-full text-xs font-bold bg-primary/15 text-primary tabular-nums">
                  {pendingCount}
                </span>
              )}
            </button>
