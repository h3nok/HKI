...
const GRADE_STYLES: Record<string, string> = {
  ...
  D: "text-primary bg-primary/10 border-primary/30",
...
function ScoreBar({ score, color }: { score: number; color: string }) {
  return (
    <div className="flex items-center gap-2 flex-1">
      ...
      <span
        className={cn("text-xs font-bold tabular-nums w-8 text-right", color)}
      >
        {(score * 100).toFixed(0)}%
      </span>
    </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground/60">
            {result.grade === "A"
              ? "Excellent — production-ready answer"
              : result.grade === "B"
                ? "Good — minor improvements possible"
                : result.grade === "C"
                  ? "Fair — some quality concerns"
                  : result.grade === "D"
