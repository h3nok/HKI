...
export default function TaxonomyTab({ selectedStream }: TaxonomyTabProps) {
  ...
  // ── Queries ──
  const rootsQ = trpc.knowledge.taxonomyListRoots.useQuery(
    explicitStreamId ? { valueStreamId: explicitStreamId } : skipToken,
    { retry: false, enabled: Boolean(explicitStreamId) }
  );
  const infoQ = trpc.knowledge.taxonomyInfo.useQuery(
    explicitStreamId ? { valueStreamId: explicitStreamId } : skipToken,
    { retry: false, enabled: Boolean(explicitStreamId) }
  );

  const childrenQ = trpc.knowledge.taxonomyGetChildren.useQuery(
    explicitStreamId && selectedNode
      ? { nodeId: selectedNode.id, valueStreamId: explicitStreamId }
      : skipToken,
    { enabled: Boolean(explicitStreamId && selectedNode), retry: false }
  );
  ...
  const handleCreate = () => {
    ...
    if (!explicitStreamId)
      return notify({ title: "Select a domain", severity: "warning" });
  ...
  const handleValidate = () => {
    ...
    if (!explicitStreamId)
      return notify({ title: "Select a domain", severity: "warning" });
    validateMut.mutate({ tags, valueStreamId: explicitStreamId });
  };
