...
function getDemoPreset(stream?: string, streamLabel?: string): DemoPreset {
  ...
  const name = streamLabel ?? "this domain";
...
function EntityGraphCore({
  ...
  const rootsQ = trpc.knowledge.taxonomyListRoots.useQuery(
    selectedStream ? { valueStreamId: selectedStream } : skipToken,
    {
      retry: false,
      staleTime: 30_000,
            ...
            <div className="rounded-2xl border border-border/30 bg-card/50 p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <p className="text-sm leading-relaxed text-muted-foreground">
                  {liveCount > 0
                    ? "Enter a business term to see how entities, policies, and concepts connect across your domain library."
