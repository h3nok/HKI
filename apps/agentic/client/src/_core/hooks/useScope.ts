/**
 * useScope — Domain scope hook for the Agentic Platform.
 *
 * Fetches domain scopes from the database via tRPC (no hardcoded data).
 *
 * Scope determines:
 ...
 *
 * Admins can switch to any domain. Other users see only assigned domains.
...
export function useScope(): ScopeState {
  // Fetch domain scopes from DB (all authenticated users can call this)
